...
var thisIndex = 'error_pv';
var reportTypeCnName,reportTypeEnName;
var keywords = '';
var hitResult = '';

var flag = true, deviceModelArr = [], tabs = {'error_pv':'error_pv','error_uv':'error_uv','pv':'pv','uv':'uv'};
...
$(function(){
    ...
    $('.tableMode a').click(function(){
    	...
    	if(thisMode == 'export'){
    		var _norm = 'error_pv';
    		if(thisIndex == 'error_pv'){
    			var _norm = 'error_pv';
    		}else if(thisIndex == 'error_uv'){
    			var _norm = 'error_uv';
...
function getData(){
	...
	$('#Datagrid').DatagridContenter({
		...
		url: projectName+'/feedbackanalysis/multiDimensionAction.do?method=get'+reportTypeEnName+'Dim&reportName='+reportName,
		height: 400,
...
function getTrendLine(){
	...
	var diff = {'Location':{'dim':'city','table':'REPORT_LOCATION_STAT_ETL'},//地理位置
				...
				'ErrorDownload':{'dim':'error_download','table':'REPORT_ERRORDOWNLOAD_STAT_ETL'},//下载错误
				'ErrorInstall':{'dim':'error_install','table':'REPORT_ERRORINSTALL_STAT_ETL'}//安装错误
	           };
	...
	$.ajax({
		...
		url: projectName+"/feedbackanalysis/multiDimensionAction.do?method=getTrendData&reportName=MultiDimensionData",		
		data: data,
		dataType : 'json',
