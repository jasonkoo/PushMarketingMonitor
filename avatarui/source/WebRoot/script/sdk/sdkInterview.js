...
var normObj = {
	...
	'uv': '访问用户量',
	'hit_pv_rate': '投放量比例',
	'hit_uv_rate': '投放用户量比例(秒)'	
};

var summaryUrlPrefix = '/summary';
...
//	}
	...
	var method, title;
	if($('#HitPV').hasClass('active')){
		method = 'getHitPVLineJsonData';
	}else if($('#HitUV').hasClass('active')){
		method = 'getHitUVLineJsonData';
	}else if($('#PV').hasClass('active')){
		method = 'getPVLineJsonData';
	}else if($('#UV').hasClass('active')){
		method = 'getUVLineJsonData';
	}else if($('#HitPVRate').hasClass('active')){
		method = 'getHitPVRateLineJsonData';
	}else if($('#HitUVRate').hasClass('active')){
		method = 'getHitUVRateLineJsonData';
	}
	...
	if($('#TimeCompareCalendar').is(':visible')){
		...
		if($('#HitPV').hasClass('active')){
			method = 'getHitPVTrendCompareData';
		}else if($('#HitUV').hasClass('active')){
			method = 'getHitUVTrendCompareData';
		}else if($('#PV').hasClass('active')){
			method = 'getPVTrendCompareData';
		}else if($('#UV').hasClass('active')){
			method = 'getUVTrendCompareData';
		}else if($('#HitPVRate').hasClass('active')){
			method = 'getHitPVRateTrendCompareData';
		}else if($('#HitUVRate').hasClass('active')){
			method = 'getHitUVRateTrendCompareData';
		}
...
//		staticData.push(_realTime);
		...
		for(var i = 0, len = twoDay.length; i < len; i++){
			var dayArr = [];
			dayArr.push(formatNum(new Number(twoDay[i].hit_pv)));
			dayArr.push(formatNum(new Number(twoDay[i].hit_uv)));
			dayArr.push(formatNum(new Number(twoDay[i].pv)));
			dayArr.push(formatNum(new Number(twoDay[i].uv)));
			dayArr.push(twoDay[i].hit_pv_rate);
			dayArr.push(twoDay[i].hit_uv_rate);
			staticData.push(dayArr);
		}
		...
		var _columns;
			...
			_columns = [
			         	{field:'hit_pv', title:'投放量', width:80, align:'center'},
						{field:'hit_uv', title:'投放用户量', width:80, align:'center'},
						{field:'pv', title:'访问量', width:80, align:'center'},
						{field:'uv', title:'访问用户量', width:80, align:'center'},
						{field:'hit_pv_rate', title:'投放量比例', width:80, align:'center'},
						{field:'hit_uv_rate', title:'投放用户量比例', width:110, align:'center'}
					];
...
*/
	...
	var _columns = [
					{field:'date', title:'时间', width:100, align:'center'},
					{field:'hit_pv', title:'投放量', width:100, align:'left', formatter:function(row){   
							return formatNum(new Number(row.hit_pv));
						}   
					},
					{field:'hit_uv', title:'投放用户量', width:100, align:'left', formatter:function(row){   
							return formatNum(new Number(row.hit_uv));
						}   
					},
					{field:'pv', title:'访问量', width:100, align:'left', formatter:function(row){   
							return formatNum(new Number(row.pv));
						}   
					},
					{field:'uv', title:'访问用户量', width:100, align:'left', formatter:function(row){   
							return formatNum(new Number(row.uv));
						}   
					},
					{field:'hit_pv_rate', title:'投放量比例', width:100, align:'left', formatter:function(row){   
						return formatNum(row.hit_pv_rate)+'%';
					    }   
				    },
				    {field:'hit_uv_rate', title:'投放用户量比例', width:100, align:'left', formatter:function(row){   
						return formatNum(row.hit_uv_rate)+'%';
					    }   
				    }					
				];
